...
try:
    ...
    raise SystemExit("linearmodels not installed. Run: uv add 'tidyecon[linearmodels]'") from err
...
fit_fe = PanelOLS.from_formula("y ~ x + EntityEffects + TimeEffects", data=df).fit(
...
fit_iv = IV2SLS.from_formula("y ~ 1 + [x ~ z]", data=df.reset_index()).fit(cov_type="robust")
